...
// hexDumpThreshold is the max number of bytes to show in hex dump before truncating.
const hexDumpThreshold = 32
...
func decodeDataPayload(portnum pb.PortNum, data []byte) string {
	...
	default:
		...
		return fmt.Sprintf("%s... (%d bytes)", hex.EncodeToString(data[:hexDumpThreshold]), len(data))
	}
}
